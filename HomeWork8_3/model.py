...
from view import (get_employee, get_changes, get_id, get_first_name, get_last_name,
                get_phone_number, get_position,get_salary,get_salary_range,
                show_search_menu, no_employee_error)
...
def write_csv(employeers: list):
    with open(Path.cwd() /'db.csv', 'w', encoding='utf-8') as fout:
...
def update_eployee(employees: list, employee: dict):
    idx = get_changes()
    if idx == 1:
        employee["id"] = get_id()
    elif idx == 2:
